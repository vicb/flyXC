...
describe('fastExtractId', () => {
  ...
  it('should skip a short id token and match a later valid one', () => {
    const line = 'FOO>BAR:/123456h idXY id253C6742 -100fpm';
    expect(fastExtractId(line)).toBe('3C6742');
  });

  it('should skip an id token followed by non-alnum and match a later valid one', () => {
    const line = 'FOO>BAR:/123456h id id1C40FE0B52 +000fpm';
    expect(fastExtractId(line)).toBe('FE0B52');
  });

  it('should return undefined when all id tokens are too short', () => {
    const line = 'FOO>BAR:/123456h idAB idCD idEF';
    expect(fastExtractId(line)).toBeUndefined();
  });

  it('should match the first valid id when multiple valid ones exist', () => {
    const line = 'FOO>BAR:/123456h id253C6742 id1C40FE0B52 +000fpm';
    expect(fastExtractId(line)).toBe('3C6742');
  });

  it('should skip bare " id" (no trailing alnum) and match a later valid one', () => {
    // The full token after ` id` is `25AABB11` (8 chars), last 6 = `AABB11`
    const line = 'FOO>BAR:/123456h id id id25AABB11 +000fpm';
    expect(fastExtractId(line)).toBe('AABB11');
  });
});
