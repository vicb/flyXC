...
// openaip.net file.
const aipFiles = glob.sync(program.opts().input + '/airspaces.json');
...
// Ukraine.
const ukraineFiles = glob.sync(program.opts().input + '/UKRAINE (UK).txt');
...
// Reunion
const reunionFile = glob.sync(program.opts().input + '/../assets/fixedasp/reunion.txt');
...
fs.writeFileSync(`${program.opts().output}.geojson`, JSON.stringify(geoJson));
    ...
    switch (m[1]) {
      ...
      case 'DC': // Circle
        ...
        for (let angle = 0; angle <= 360; angle += 10) {
          const { latitude, longitude } = computeDestinationPoint(center, NM * parseFloat(m[2]), angle);
          coords.push([longitude, latitude]);
        }
        break;
