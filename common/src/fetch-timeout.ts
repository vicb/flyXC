 ...
 */
export async function fetchResponse(url: string, options?: { retry?: number; timeoutS?: number }): Promise<Response> {
  const { retry = 3, timeoutS = 5 } = options ?? {};
  const signal = (AbortSignal as any).timeout(timeoutS * 1000);
  ...
  for (let numRetry = 0; numRetry < retry; numRetry++) {
    try {
      ...
      if (e?.name == 'AbortError' || e?.name == 'TimeoutError') {
        throw new Error(`Timeout ${timeoutS}s`);
      }
