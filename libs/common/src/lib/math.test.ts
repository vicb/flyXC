...
describe('findFirstIndex', () => {
  describe('Comparison.GREATER_EQUAL', () => {
    ...
    it('should handle single element arrays', () => {
      expect(findFirstIndex([10], 5, cmp)).toBe(0);
      expect(findFirstIndex([10], 10, cmp)).toBe(0);
      expect(findFirstIndex([10], 15, cmp)).toBe(1);
    });
    ...
    it('should return first index when duplicate values exist', () => {
      ...
      expect(findFirstIndex(arr, 20, cmp)).toBe(1);
    });
  });

  describe('Comparison.GREATER', () => {
    const cmp = Comparison.GREATER;
    ...
    it('should handle single element arrays', () => {
      expect(findFirstIndex([10], 5, cmp)).toBe(0);
      expect(findFirstIndex([10], 10, cmp)).toBe(1);
      expect(findFirstIndex([10], 15, cmp)).toBe(1);
    });

    it('should find upper bound in multi-element arrays', () => {
      const list = [10, 20, 30, 40, 50];
      expect(findFirstIndex(list, 5, cmp)).toBe(0);
      expect(findFirstIndex(list, 10, cmp)).toBe(1);
      expect(findFirstIndex(list, 15, cmp)).toBe(1);
      expect(findFirstIndex(list, 20, cmp)).toBe(2);
      expect(findFirstIndex(list, 25, cmp)).toBe(2);
      expect(findFirstIndex(list, 30, cmp)).toBe(3);
      expect(findFirstIndex(list, 40, cmp)).toBe(4);
      expect(findFirstIndex(list, 50, cmp)).toBe(5);
      expect(findFirstIndex(list, 55, cmp)).toBe(5);
    });
