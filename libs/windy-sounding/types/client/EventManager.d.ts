...
export declare class EventManager {
   ...
   */
  addDOMListener<T extends EventTarget>(
    target: T,
    topic: Parameters<T['addEventListener']>[0],
    callback: Parameters<T['addEventListener']>[1],
  ): void;
  /**
   ...
   * @param callback  - callback function that should process the event
   */
  addEventedListener<T>(
    target: Evented<T>,
    topic: keyof T,
    callback: (typeof this.eventedCallbacks)[string][number]['ref'],
  ): void;
  /**
