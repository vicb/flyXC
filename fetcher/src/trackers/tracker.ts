...
export interface TrackerUpdates {
  ...
  trackerDeltas: Map<number, LiveTrack>;
...
export class TrackerFetcher {
  ...
  async refresh(fetchTimeoutSec: number): Promise<TrackerUpdates> {
    const updates: TrackerUpdates = {
      ...
      errors: [],
      trackerDeltas: new Map<number, LiveTrack>(),
  ...
  protected getTracker(id: number): Tracker | undefined {
    const pilot = this.state.pilots[String(id)];
    if (!pilot) {
      return undefined;
    }
    return (pilot as any)[this.propName] as Tracker | undefined;
  }
  ...
  private incrementRequest(tracker: Tracker, hasError: boolean) {
    tracker.numRequests++;
  ...
  private getDevicesToFetch(): number[] {
    ...
    for (const [idStr, pilot] of Object.entries(this.state.pilots)) {
      ...
      if (pilot.enabled) {
        const tracker = (pilot as any)[propName] as Tracker | undefined;
