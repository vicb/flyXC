...
import { LIVE_REFRESH_SEC, trackerPropNames } from 'flyxc/common/src/live-track';
...
export function syncLiveTrack(state: FetcherState, liveTrack: LiveTrackEntity) {
  const id = liveTrack[Datastore.KEY].id;
  ...
  if (statePilot != null) {
    ...
    for (const propName of Object.values(trackerPropNames)) {
      const tracker = (pilot as any)[propName] as Tracker;
      const stateTracker = (statePilot as any)[propName] as Tracker;
...
function createPilotFromEntity(liveTrack: LiveTrackEntity): Pilot {
  const e = liveTrack;
