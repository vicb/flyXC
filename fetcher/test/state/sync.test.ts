...
import { LiveTrackEntity, TrackerEntity } from 'flyxc/common/src/live-track-entity';
import { createInitState } from 'flyxc/fetcher/src/state/state';
import { syncLiveTrack } from 'flyxc/fetcher/src/state/sync';
...
describe('sync', () => {
  ...
  describe('Updating an existing pilot', () => {
    ...
    it('keeps track when no update', () => {
      ...
      const lt = createLiveTrackEntity('1978');
      syncLiveTrack(state, lt);
      state.pilots[1978].track = LiveTrack.create({ timeSec: [1, 2, 3] });
